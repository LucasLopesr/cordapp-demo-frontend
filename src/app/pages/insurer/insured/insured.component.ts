...
export class InsurerInsuredComponent implements OnInit {
  ...
  displayedColumns = ['seller', 'product', 'insurance', 'date', 'billed', 'comission'];
  ...
  private requestData() {
    this.insuranceService.getInsurancesFromInsurer().subscribe(insurances => {
  ...
  createRows(insurances: Insurance[]) {
    ...
    insurances.forEach(insurance => {
      rows.push({
        seller: this.getChannel(insurance.seller),
  ...
  getChannel(seller: string): string {
    return seller === 'Varejista1' ? 'A' : 'B';
  }
